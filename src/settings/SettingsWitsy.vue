...
const load = () => {
  apiKey.value = store.config.engines.witsy?.apiKey || ''
  chat_models.value = store.config.engines.witsy?.models?.chat || []
  chat_model.value = store.config.engines.witsy?.model?.chat || ''
  disableTools.value = store.config.engines.witsy?.disableTools || false
}
...
const getModels = async () => {
  ...
  let success = await llmManager.loadModels('witsy')
  if (!success) {
...
const onKeyChange = () => {
  if (chat_models.value.length === 0 && apiKey.value.length > 0) {
    store.config.engines.witsy.apiKey = apiKey.value
    getModels()
  }
  save()
}
...
const save = () => {
  store.config.engines.witsy.apiKey = apiKey.value
  store.config.engines.witsy.model.chat = chat_model.value
  store.config.engines.witsy.disableTools = disableTools.value
  store.saveSettings()
}
